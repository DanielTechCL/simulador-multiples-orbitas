...
for i in range(num_orbitas):
    radio = float(input('Radio para órbita {i=1} (km): '))
    angulos = np.linspace(0, 2*np.pi, 1000)

    if radio <= 2000:
        print('Órbita {i=1}: Baja sobre el mar')
    elif radio >= 1500001:
        print('Órbita {i=1}: Fuera de Órbita')
    elif radio >= 35787:
        print('Órbita {i=1}: Alta sobre el mar')
    elif radio >= 2001:
        print('Órbita {i=1}: Media sobre el mar')
